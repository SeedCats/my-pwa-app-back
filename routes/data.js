...
router.get('/status', authenticate, async (req, res) => {
    try {
        ...
        // Use upsert to ensure status exists - creates user doc if needed
        const userDoc = await db.collection('user').findOneAndUpdate(
            { _id: targetUserId },
            { 
                $setOnInsert: { status: 'On-going', statusUpdatedAt: now },
                $set: { lastAccessedAt: now }
            },
            { upsert: true, returnDocument: 'after' }
        );

        res.status(200).json({ success: true, message: 'Status fetched', data: { status: userDoc.status, updatedAt: userDoc.statusUpdatedAt, userId: String(userDoc._id) } });
...
router.post('/status/complete', authenticate, async (req, res) => {
    try {
        ...
        const result = await db.collection('user').updateOne(
            { _id: targetUserId },
            { $set: { status: 'Completed', statusUpdatedAt: now } }
        );
